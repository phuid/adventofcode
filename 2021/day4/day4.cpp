...
int main()
{
  ...
  for (size_t i = 2; i < inputvector.size(); i++)
  {
    if (inputvector[i].length() > 0)
    {
      ...
      while (inputvector[i].length() > 0)
      {
        ...
        {
          boards.back().back().push_back({stoi(inputvector[i].substr(0, inputvector[i].find(' '))), 0});
  ...
  while (!gameover)
  {
    ...
    {
      for (size_t y = 0; y < boards[z].size(); y++)
      {
        bool allmarked = 1;
        for (size_t x = 0; x < boards[z][y].size(); x++)
        {
          allmarked *= boards[z][y][x].marked;
        }
    ...
    for (size_t z = 0; z < boards.size(); z++)
    {
      for (size_t x = 0; x < boards[z].size(); x++)
      {
        int allmarked = 1;
        for (size_t y = 0; y < boards[z][x].size(); y++)
        {
          allmarked *= boards[z][y][x].marked;
        }
        if (allmarked == 1)
